...
- CreoSource : based on BGRAControls ColorSpeedButton

***************************** END CONTRIBUTOR(S) *****************************}
unit ColorSpeedButtonCS;
...
type
  ...
  private
    ...
    procedure SetFBorderWidth(AValue: TColor);
  ...
  published
    ...
    property FontColor: TColor read FFontColor write SetFBorderWidth;
  end;
  ...
  TColorSpeedButtonCS = class(TBGRASpeedButton)
  private
    ...
    procedure SetTextPadding(AValue: integer);
  protected
    {$ifdef overridepaint}
...
procedure Register;
begin
  RegisterComponents('CS', [TColorSpeedButtonCS]);
end;
{$ENDIF}

{ TColorSpeedButton }

procedure TColorSpeedButtonCS.SetFStateActive(AValue: TColorState);
begin
...
procedure TColorSpeedButtonCS.SetFPopupMode(AValue: boolean);
begin
...
procedure TColorSpeedButtonCS.SetFPressed(AValue: boolean);
begin
...
procedure TColorSpeedButtonCS.SetFStateDisabled(AValue: TColorState);
begin
...
procedure TColorSpeedButtonCS.SetFStateHover(AValue: TColorState);
begin
...
procedure TColorSpeedButtonCS.SetFStateNormal(AValue: TColorState);
begin
...
procedure TColorSpeedButtonCS.SetFTextAutoSize(AValue: boolean);
begin
...
procedure TColorSpeedButtonCS.SetFToggle(AValue: boolean);
begin
...
procedure TColorSpeedButtonCS.SetTextPadding(AValue: integer);
begin
...
{$ifdef overridepaint}
procedure TColorSpeedButtonCS.DrawText(ACanvas: TPersistent;
...
begin
  ...
  case TempState of
    bsUp:
    begin
      Canvas.Font.Color := FStateNormal.FontColor;
    end;
    bsDisabled:
    begin
      Canvas.Font.Color := FStateDisabled.FontColor;
    end;
    bsDown, bsExclusive:
    begin
      Canvas.Font.Color := FStateActive.FontColor;
    end;
    {$IFDEF FPC}//#
    bsHot:
    begin
      Canvas.Font.Color := FStateHover.FontColor;
    end;
    {$ENDIF}
  end;
...
procedure TColorSpeedButtonCS.MeasureDraw(Draw: boolean; PaintRect: TRect;
  out PreferredWidth, PreferredHeight: integer);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if (FAlignment = TAlignment.taLeftJustify) then
        begin
          Left := Left + FTextPadding;
          Top := Top + Y;
        end
        else if (FAlignment = TAlignment.taCenter) then
        begin
          Left := Left + X;
          Top := Top + Y;
        end
        else if (FAlignment = TAlignment.taRightJustify) then
        begin
          Right := Right - FTextPadding;
          Top := Top + Y;
        end
      end;
...
procedure TColorSpeedButtonCS.Paint;
var
  PaintRect: TRect;
...
procedure TColorSpeedButtonCS.CalculatePreferredSize(
...
procedure TColorSpeedButtonCS.PaintBackground(var PaintRect: TRect);
var
  TempState: TButtonState;
begin
  TempState := FState;
...
procedure TColorSpeedButtonCS.SetAlignment(Value: TAlignment);
begin
...
constructor TColorSpeedButtonCS.Create(TheOwner: TComponent);
begin
...
procedure TColorSpeedButtonCS.Click;
var
  p: TPoint;
begin
  if Toggle then
...
procedure TColorState.SetFBorderWidth(AValue: TColor);
begin
...
procedure TColorState.SetFontColor(AValue: TColor);
begin
